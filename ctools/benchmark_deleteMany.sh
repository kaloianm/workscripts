...
# (34M docs, 10 secondary indexes), then times a deleteMany of ~9M documents
# (~100GB) in a contiguous shardKey range.
#
# Designed to be run alongside benchmark_fastBulkDelete.sh on a separate
# cluster for an apples-to-apples comparison.
##############################################################################

##############################################################################
# Configuration — edit these as needed
##############################################################################
CLUSTER_TAG="bench-deleteMany"
...
##############################################################################
# Step 7: Run deleteMany (TIMED)
##############################################################################
echo ""
echo "=== Step 7: Running deleteMany — shardKey [${DELETE_LOWER}, ${DELETE_UPPER}) ==="
DELETE_START=$(date +%s)

mongosh --quiet "mongodb://${RS_HOST}:27017/locust_read_write_load" --eval "
    var start = new Date();
    var result = db.load.deleteMany({shardKey: {\$gte: ${DELETE_LOWER}, \$lt: ${DELETE_UPPER}}});
    var end = new Date();
    printjson({
...
echo ""
echo ">>> deleteMany wall-clock runtime: ${WALL_CLOCK} seconds <<<"
...
echo ""
echo "================================================================"
echo "  BENCHMARK COMPLETE: deleteMany"
echo "================================================================"
echo "  Cluster tag:       ${CLUSTER_TAG}"
